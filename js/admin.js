...
function changeOrderStatus(id, status) {
  ...
  fetch(`${Utils.API_BASE_URL}/orders/${id}/status`, {
    method: 'PUT',
...
function saveProduct() {
  ...
  if (id) {
    ...
    fetch(`${Utils.API_BASE_URL}/products/${product.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    Admin.saveProducts(window._adminProducts);
    fetch(`${Utils.API_BASE_URL}/products`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(product)
    }).catch(() => {});
...
function deleteProduct(id) {
  ...
  Admin.saveProducts(window._adminProducts);
  fetch(`${Utils.API_BASE_URL}/products/${id}`, { method: 'DELETE' }).catch(() => {});
  renderProductsTable();
  renderStats(Admin.getAllOrders(), window._adminProducts);
...
async function initAdmin() {
  ...
  // 1. Load products dari Railway API atau fallback
  try {
    const res = await fetch(`${Utils.API_BASE_URL}/products`);
    if (res.ok) {
      const result = await res.json();
      if (result.success && result.data) {
        window._adminProducts = result.data;
      }
    }
  } catch (err) {
    console.warn('⚡ API Backend tidak terjangkau untuk produk admin, menggunakan lokal...');
  }
  ...
  // 2. Load orders dari Railway API atau fallback
  try {
...
function renderCategories() {
  ...
  if (!grid) return;
  grid.innerHTML = Object.entries(catCounts).map(([cat, count]) => `
    <div class="bg-gray-50 dark:bg-gray-700 rounded-2xl p-4 flex items-center justify-between shadow-sm">
      <div>
        <h4 class="font-bold text-gray-800 dark:text-white text-sm">${cat}</h4>
        <p class="text-xs text-green-600 font-semibold">${count} Produk Aktif</p>
      </div>
...
function addCategory() {
  ...
  if (!name) { Utils.showToast('Nama kategori wajib diisi.', 'error'); return; }
  const dummy = {
    id: Date.now(),
    name: `Contoh Produk (${name})`,
    price: 50000,
    category: name,
    stock: 10,
    rating: 5.0,
    image: 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=400&auto=format&fit=crop',
    description: `Produk dalam kategori ${name}`
  };
  window._adminProducts.push(dummy);
  Admin.saveProducts(window._adminProducts);
...
async function syncWithRailway() {
  ...
  await initAdmin();
  Utils.showToast('✅ Data berhasil tersinkronisasi 100% dari Railway API!', 'success');
}

function saveStoreSettings() {
  const wa = document.getElementById('store-wa')?.value || '';
  localStorage.setItem('tektok_store_wa', wa);
  Utils.showToast('💾 Pengaturan toko & nomor WA berhasil disimpan!', 'success');
}
